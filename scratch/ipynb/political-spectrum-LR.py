...
for status in data:
    ...
    if status.label == "libs" or status.label == "cons" or status.label == "p":
        LABELS.append("p")
    else:
        LABELS.append("np")
...
def average_score(probs):
    ...
    for prob in probs:
        if prob.item(0) > prob.item(1):
            score += prob.item(0)
        else:
            score += prob.item(1)

    # higher probabilities in logistic regression indicate np timeline, whereas my d3 visualization assumes the reverse
    # ndarray ordered lexigraphically (np before p)
    average_score = 1 - (score/len(probs))
    print average_score
